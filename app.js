...
// ✅ Validación ±5% INTA
const tolerancia = validarToleranciaDensidad(densidadObj, dens_corregida);
    ...
    <div class="result">
        ...
        <hr>
<br><b>Tolerancia INTA (±5%):</b> ${tolerancia.dentro ? '✅ DENTRO' : '❌ FUERA'} (${tolerancia.limiteInf} - ${tolerancia.limiteSup} sem/ha)
...
// ===================================================================
// VALIDACIÓN ±5% DENSIDAD (INTA)
// ===================================================================
function validarToleranciaDensidad(densidadObj, densidadCorregida) {
  const limiteInf = Math.round(densidadObj * 0.95);
  const limiteSup = Math.round(densidadObj * 1.05);
  const dentro = densidadCorregida >= limiteInf && densidadCorregida <= limiteSup;
  return { dentro, limiteInf, limiteSup };
}

// ===================================================================
// BOTÓN "VER FÓRMULA INTA"
// ===================================================================
function mostrarFormulaINTA() {
  const htmlFormula = `
    <div id="formulaINTA" class="result" style="margin-top:12px; background:#f0f9ff; border:1px solid #0ea5e9; padding:12px; border-radius:8px;">
      <h4 style="margin:0 0 8px 0; color:#0ea5e9;">📋 Fórmulas INTA / ISO 7256/1</h4>
      <ol style="margin:0; padding-left:18px; font-size:0.9em; color:#334155;">
        <li><strong>Dref (cm)</strong> = (10000 × 100) / (e × densidadObj)</li>
        <li><strong>Falla simple</strong> &gt; 1.5 × Dref</li>
        <li><strong>Falla doble</strong> &gt; 2.5 × Dref</li>
        <li><strong>Duplicación</strong> &lt; 0.5 × Dref</li>
        <li><strong>Eventos corregidos</strong> = Totales + Fallas + 2×Dobles − Duplicaciones</li>
        <li><strong>Densidad corregida</strong> = (Eventos corregidos × 10000) / (tramo × e)</li>
        <li><strong>Tolerancia densidad</strong> ±5% respecto a densidadObj</li>
      </ol>
      <button class="btn-secondary" style="margin-top:8px;" onclick="cerrarFormulaINTA()">Cerrar</button>
    </div>
  `;
  const salida = document.getElementById('salida');
  salida.insertAdjacentHTML('afterend', htmlFormula);
}

function cerrarFormulaINTA() {
  document.getElementById('formulaINTA').remove();
}

// ===================================================================
// TEST OFICIAL INTA (63 eventos)
// ===================================================================
function testOficialINTA() {
  const datosOficial = [
    22,20,18,18,22,23,35,20,16,24,16,21,24,53,12,36,16,18,14,22,22,14,14,23,19,20,20,17,21,21,18,57,22,19,23,20,22,13,20,17,20,22,61,37,38,21,16,36,20,18,23,38,18,21,14,23,19,20,23,17,4,14,17,16,22,40,13];
  document.getElementById('densidadObj').value = 70000;
  document.getElementById('distSurco').value = 70;
  document.getElementById('tramo').value = 14.3;
  document.getElementById('distancias').value = datosOficial.join(', ');
  calcular(); // ejecuta el cálculo
  alert('✅ Test oficial INTA cargado. Verifica:\nDref = 20.4 cm\nEventos corregidos = 74\nDensidad corregida = 74 000 sem/ha');
}
